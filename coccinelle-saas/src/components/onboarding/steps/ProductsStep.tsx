  ...
  return (
    <div>
      <div className="flex items-center justify-center mb-4">
        ...
        <h2 className="text-2xl font-bold text-gray-900">Vos produits / services</h2>
      </div>
      ...
      {totalCount > 0 && (
        <div className="mb-6 bg-green-50 border border-green-200 rounded-lg px-4 py-3 text-center">
          <span className="text-[#0F6E56] font-semibold">{totalCount} produit{totalCount > 1 ? 's' : ''} ajoute{totalCount > 1 ? 's' : ''}</span>
        </div>
      )}
        ...
        {mode === 'choose' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            >
              ...
              <span className="font-medium text-gray-700">Importer un CSV</span>
              <p className="text-sm text-gray-500 mt-1">Importez depuis un fichier</p>
            </button>
          </div>
        )}
              ...
              <div className="mt-4 border border-gray-200 rounded-lg divide-y divide-gray-100">
                {products.map((p, i) => (
                  <div key={i} className="px-4 py-2 flex justify-between items-center">
                    ...
                    {p.price && <span className="text-sm text-gray-500">{p.price} EUR</span>}
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="text-center">
            <label className="block border-2 border-dashed border-gray-300 hover:border-[#D85A30] rounded-xl p-8 cursor-pointer transition-colors">
              ...
              <span className="text-gray-600 font-medium">
                {uploading ? 'Import en cours...' : 'Cliquez pour selectionner un fichier CSV'}
              </span>
              <input
                type="file"
                accept=".csv"
