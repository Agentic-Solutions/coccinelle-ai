...
const CHANNELS = [
  ...
  },
  {
    id: 'email',
    label: 'Email',
    description: 'Réponses email automatiques',
    icon: (
      <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  },
  {
    ...
    // Gelé (Lot 0) : /channels/whatsapp/enable renvoie 404 et l'échec n'était que
    // console.warn — le canal aurait échoué en silence dans le tunnel d'onboarding.
    comingSoon: true,
    icon: (
      <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
export default function ChannelsStep({ channelsData, onChannelsChange, onNext, onBack, onSkip }: ChannelsStepProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Gelé (Lot 0) : filtre défensif — une session entamée avant le gel pouvait
      // conserver 'whatsapp' dans channelsData et le POSTer (404 avalé en console.warn).
      const enabledChannels = channelsData.filter(c => c !== 'whatsapp');
  ...
  return (
    <div>
      ...
      <p className="text-center text-gray-500 mb-8">
        Sélectionnez les canaux que vous souhaitez activer
      </p>
      ...
      <div className="max-w-lg mx-auto grid grid-cols-1 sm:grid-cols-2 gap-4">
        {CHANNELS.map(channel => {
          const comingSoon = (channel as any).comingSoon;
          const isActive = !comingSoon && channelsData.includes(channel.id);
          return (
            <button
              key={channel.id}
              type="button"
              disabled={comingSoon}
              onClick={() => !comingSoon && toggleChannel(channel.id)}
              className={`
            ...
            >
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  <span className={`font-medium ${isActive ? 'text-[#D85A30]' : comingSoon ? 'text-gray-400' : 'text-gray-700'}`}>
                    {channel.label}
                  </span>
                  {comingSoon ? (
