...
import {
  ...
  Calendar, Search, ShoppingBag, UserPlus, PhoneForwarded,
  ArrowRight, Link2
} from 'lucide-react';
...
type NodeType = 'call' | 'sms' | 'email' | 'delay' | 'condition' | 'end' | 'rdv' | 'knowledge' | 'products' | 'prospect' | 'transfer';
...
const NODE_TYPES: Record<NodeType, {
  ...
  rdv:       { label: 'Prise de RDV',          icon: Calendar,       leftBorder: 'border-l-gray-800' },
  knowledge: { label: 'Base de connaissances', icon: Search,         leftBorder: 'border-l-gray-600' },
  products:  { label: 'Produits & Services',   icon: ShoppingBag,    leftBorder: 'border-l-gray-500' },
  prospect:  { label: 'Créer prospect',        icon: UserPlus,       leftBorder: 'border-l-gray-700' },
  transfer:  { label: 'Transfert humain',      icon: PhoneForwarded, leftBorder: 'border-l-gray-900' },
};

// ─── Node palette (catégories pour le sélecteur) ────────────────────────────
...
const NODE_PALETTE: PaletteItem[] = [
  { type: 'call',      label: 'Appel vocal',           description: "Script exact que l'agent prononce",        category: 'CONVERSATION' },
  ...
  { type: 'condition', label: 'Condition',              description: 'Brancher selon la réponse du client',     category: 'LOGIQUE' },
  { type: 'delay',     label: 'Délai',                 description: "Attendre avant l'étape suivante",         category: 'LOGIQUE' },
  { type: 'rdv',       label: 'Prise de RDV',          description: 'Vérifier les disponibilités et réserver', category: 'ACTIONS', tool: 'check_availability + book_appointment' },
  { type: 'knowledge', label: 'Base de connaissances', description: 'Chercher dans la KB du tenant',           category: 'ACTIONS', tool: 'search_knowledge' },
  { type: 'products',  label: 'Produits & Services',   description: 'Chercher dans les produits',              category: 'ACTIONS', tool: 'search_products' },
  { type: 'prospect',  label: 'Créer prospect',        description: 'Enregistrer le contact dans le CRM',     category: 'ACTIONS', tool: 'create_prospect' },
  { type: 'transfer',  label: 'Transfert humain',      description: 'Transférer vers un conseiller',           category: 'ACTIONS', tool: 'transfer_to_human' },
  { type: 'end',       label: 'Fin',                   description: 'Terminer la séquence',                    category: 'FIN' },
];

const PALETTE_CATEGORIES = ['CONVERSATION', 'LOGIQUE', 'ACTIONS', 'FIN'];
...
function buildSequence(sector: string, company: string): { nodes: SequenceNode[]; connections: Connection[] } {
  ...
  // Priorité : script (texte exact) > instruction (directive générique)
  const findNode = (name: string) => {
    const node = sp?.nodes.find(n => n.name === name);
    return node?.script || node?.instruction;
  };
...
export default function SequenceEditor() {
  ...
  const [connectingFrom, setConnectingFrom] = useState<{ nodeId: string; output: 'default' | 'yes' | 'no' } | null>(null);
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (!rect) return;
    if (connectingFrom) {
      setConnectingMouse({ x: e.clientX - rect.left, y: e.clientY - rect.top });
      return;
    }
    if (!dragging) return;
  ...
  function addNode(type: NodeType) {
    ...
    const defaults: Record<NodeType, { title: string; content: string }> = {
      call:      { title: 'Appel vocal',           content: 'Bonjour, comment puis-je vous aider ?' },
      sms:       { title: 'SMS',                   content: 'Message de suivi de {COMPANY_NAME}' },
      email:     { title: 'Email',                 content: 'Email de {COMPANY_NAME}' },
      condition: { title: 'Condition',             content: 'A exprimé un intérêt ?' },
      delay:     { title: 'Délai',                 content: 'Attente avant prochaine étape' },
      rdv:       { title: 'Prise de RDV',          content: 'Je vérifie les disponibilités...' },
      knowledge: { title: 'Base de connaissances', content: 'Je recherche l\'information...' },
      products:  { title: 'Produits & Services',   content: 'Je recherche nos produits...' },
      prospect:  { title: 'Créer prospect',        content: 'Je note vos coordonnées...' },
      transfer:  { title: 'Transfert humain',      content: 'Je vous transfère vers un conseiller.' },
      end:       { title: 'Fin de séquence',       content: 'Fin de séquence' },
    };
    ...
    const newNode: SequenceNode = {
      ...
      ...(type === 'delay' ? { delayDuration: '3d' } : {}),
      ...(type === 'condition' ? { condition: 'interested' } : {}),
    };
  ...
  function addConnection(fromId: string, toId: string, fromOutput: 'default' | 'yes' | 'no' = 'default') {
    if (fromId === toId) return;
    // Prevent duplicate connections from same output
    const exists = connections.some(c => c.from === fromId && c.to === toId && (c.fromOutput || 'default') === fromOutput);
    if (exists) return;
    const label = fromOutput === 'yes' ? 'OUI' : fromOutput === 'no' ? 'NON' : undefined;
    setConnections(prev => [...prev, { id: `c${Date.now()}`, from: fromId, to: toId, label, fromOutput }]);
  }
  ...
  function updateConnectionLabel(connId: string, label: string) {
    setConnections(prev => prev.map(c => c.id === connId ? { ...c, label: label || undefined } : c));
  }

  function startConnecting(nodeId: string, output: 'default' | 'yes' | 'no', e: React.MouseEvent) {
    e.stopPropagation();
    e.preventDefault();
    const rect = canvasRef.current?.getBoundingClientRect();
    if (!rect) return;
  ...
  function finishConnecting(targetNodeId: string, e: React.MouseEvent) {
    e.stopPropagation();
    e.preventDefault();
    if (connectingFrom && connectingFrom.nodeId !== targetNodeId) {
      addConnection(connectingFrom.nodeId, targetNodeId, connectingFrom.output);
    }
    setConnectingFrom(null);
    setConnectingMouse(null);
  }
      ...
      <div className="flex h-[calc(100vh-73px)]">
        ...
        <div className="flex-1 overflow-auto relative bg-[#f8f9fb]">
          ...
          >
            ...
            {/* Connection lines */}
            <svg className="absolute inset-0 w-full h-full" style={{ minHeight: canvasHeight, zIndex: 5 }}>
                ...
                return (
                  <g key={conn.id} className="group/conn">
                    ...
                    <path
                      ...
                      className="pointer-events-none group-hover/conn:stroke-gray-500 transition-colors"
                    />
                    <circle cx={to.x} cy={to.y} r="4" fill="#d1d5db" className="pointer-events-none" />
                    {conn.label && (
                      <g>
                        <rect
                          x={midX - 18}
              ...
              {/* Temp line while dragging to connect */}
              {connectingFrom && connectingMouse && (() => {
                const fromNode = nodes.find(n => n.id === connectingFrom.nodeId);
                if (!fromNode) return null;
                const from = getOutputPoint(fromNode, connectingFrom.output);
                const path = buildPath(from, connectingMouse);
                return (
                  <path
                    d={path}
                    fill="none"
              ...
              return (
                ...
                >
                  <div className={`rounded-lg border border-gray-200 border-l-2 ${cfg.leftBorder} ${
                    ...
                    {/* Input connector (top) */}
                    <div
                      className={`absolute -top-2 left-1/2 -translate-x-1/2 w-4 h-4 rounded-full bg-white border-2 z-10 transition-all ${
                        connectingFrom ? 'border-gray-600 hover:border-gray-900 hover:bg-gray-100 hover:scale-125 cursor-crosshair' : 'border-gray-300'
                      }`}
                      onMouseUp={(e) => finishConnecting(node.id, e)}
                    />
                    {/* Output connectors (bottom) */}
                    {node.type === 'condition' ? (
                      <>
                        <div
                          className="absolute -bottom-2 left-1/4 -translate-x-1/2 w-4 h-4 rounded-full bg-white border-2 border-gray-400 z-10 hover:border-gray-900 hover:bg-gray-100 hover:scale-125 cursor-crosshair transition-all"
                          title="OUI"
                          onMouseDown={(e) => startConnecting(node.id, 'yes', e)}
                        />
                        <div
                          className="absolute -bottom-2 left-3/4 -translate-x-1/2 w-4 h-4 rounded-full bg-white border-2 border-gray-400 z-10 hover:border-gray-900 hover:bg-gray-100 hover:scale-125 cursor-crosshair transition-all"
                          title="NON"
                          onMouseDown={(e) => startConnecting(node.id, 'no', e)}
                        />
                      </>
                    ) : node.type !== 'end' ? (
                      <div
                        className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-4 h-4 rounded-full bg-white border-2 border-gray-300 z-10 hover:border-gray-900 hover:bg-gray-100 hover:scale-125 cursor-crosshair transition-all"
                        onMouseDown={(e) => startConnecting(node.id, 'default', e)}
                      />
                    ) : null}
                  </div>
                </div>
            ...
            <div className="p-5 space-y-5">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  {selected.type === 'call' ? 'Script exact' :
                   selected.type === 'sms' ? 'Contenu SMS' :
                   selected.type === 'email' ? 'Contenu email' :
                   selected.type === 'condition' ? 'Description' :
                   selected.type === 'delay' ? 'Note' :
                   selected.type === 'rdv' ? 'Script RDV' :
                   selected.type === 'knowledge' ? 'Script recherche KB' :
                   selected.type === 'products' ? 'Script recherche produits' :
                   selected.type === 'prospect' ? 'Script enregistrement' :
                   selected.type === 'transfer' ? 'Script transfert' :
                   'Description'}
                </label>
                <textarea
                ...
                {selected.type === 'call' && (
                  <p className="text-xs text-blue-500 mt-1">Modifiez ce texte directement — c&apos;est exactement ce que l&apos;agent vocal dira.</p>
                )}
                {['rdv', 'knowledge', 'products', 'prospect', 'transfer'].includes(selected.type) && (
                  <p className="text-xs text-blue-500 mt-1">Ce noeud appelle automatiquement l&apos;outil connect&eacute;.</p>
                )}
              </div>
              ...
              {/* Tool indicator for action nodes */}
              {['rdv', 'knowledge', 'products', 'prospect', 'transfer'].includes(selected.type) && (
                <div className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                  <p className="text-xs font-semibold text-gray-700 mb-1.5">Outil connect&eacute; :</p>
                  <p className="text-xs text-gray-600 font-mono">
                    {selected.type === 'rdv' && 'check_availability + book_appointment'}
                    {selected.type === 'knowledge' && 'search_knowledge'}
                    {selected.type === 'products' && 'search_products'}
                    {selected.type === 'prospect' && 'create_prospect'}
                    {selected.type === 'transfer' && 'transfer_to_human'}
                  </p>
                  <p className="text-[10px] text-gray-400 mt-1">Endpoint API appel&eacute; automatiquement lors de l&apos;ex&eacute;cution.</p>
                </div>
              )}
              ...
              <div>
                ...
                {(() => {
                  const outgoing = connections.filter(c => c.from === selected.id);
                  const incoming = connections.filter(c => c.to === selected.id);
                  const otherNodes = nodes.filter(n => n.id !== selected.id);
                  return (
                    <div className="space-y-3">
                      {outgoing.length > 0 ? (
                        <div className="space-y-2">
                          <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Sorties</p>
                          {outgoing.map(conn => {
                            const target = nodes.find(n => n.id === conn.to);
                            return (
                              <div key={conn.id} className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg border border-gray-200">
                                ...
                                <div className="flex-1 min-w-0">
                                  <input
                                    ...
                                    placeholder="Label..."
                                    className="w-full px-2 py-1 text-xs border border-gray-200 rounded focus:ring-1 focus:ring-gray-900 focus:border-transparent"
                                  />
                                </div>
                                <select
                                  value={conn.to}
                                  onChange={(e) => {
                                    setConnections(prev => prev.map(c => c.id === conn.id ? { ...c, to: e.target.value } : c));
                                  }}
                                  className="text-xs border border-gray-200 rounded px-1.5 py-1 max-w-[100px] focus:ring-1 focus:ring-gray-900"
                                >
                                  {otherNodes.map(n => (
                      ...
                      {incoming.length > 0 ? (
                        <div className="space-y-2">
                          <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Entrées</p>
                          {incoming.map(conn => {
                            const source = nodes.find(n => n.id === conn.from);
                            return (
                              <div key={conn.id} className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg border border-gray-200">
                                <ArrowRight className="w-3.5 h-3.5 text-gray-400 flex-shrink-0 rotate-180" />
                                <span className="text-xs text-gray-600 truncate flex-1">{source?.title || conn.from}</span>
                                {conn.label && <span className="text-[10px] px-1.5 py-0.5 bg-gray-200 rounded text-gray-600">{conn.label}</span>}
                                <button
                                  onClick={() => deleteConnection(conn.id)}
                                  className="p-1 hover:bg-gray-200 rounded transition-colors flex-shrink-0"
                                  title="Supprimer"
                      ...
                      {outgoing.length === 0 && incoming.length === 0 && (
                        <p className="text-xs text-gray-400 py-2">Aucune connexion. Glissez depuis un connecteur pour relier ce noeud.</p>
                      )}
                      {/* Add transition button */}
                      {selected.type !== 'end' && otherNodes.length > 0 && (
                        <button
                          onClick={() => {
                            const target = otherNodes[0];
                            addConnection(selected.id, target.id, selected.type === 'condition' ? 'yes' : 'default');
                          }}
                          className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-700 font-medium transition-colors mt-1"
                        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 max-h-[80vh] overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-gray-900">Choisir un type de noeud</h3>
              <button
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(80vh-65px)]">
              {PALETTE_CATEGORIES.map(cat => {
                const items = NODE_PALETTE.filter(p => p.category === cat);
                return (
                  <div key={cat} className="mb-6 last:mb-0">
                    <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">{cat}</h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {items.map(item => {
                        const cfg = NODE_TYPES[item.type];
                        const Icon = cfg.icon;
                        return (
                          <button
                            key={item.type}
                            onClick={() => addNode(item.type)}
                            className="flex items-start gap-3 p-4 rounded-xl border border-gray-200 hover:border-gray-400 hover:bg-gray-50 transition-all text-left group"
                          >
                            ...
                            <div className="min-w-0">
                              ...
                              {item.tool && (
                                <p className="text-[10px] text-gray-400 mt-1 font-mono">{item.tool}</p>
                              )}
                            </div>
                          </button>
                        );
