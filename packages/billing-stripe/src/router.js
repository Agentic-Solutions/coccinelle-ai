...
/**
 * Créer un router billing configuré
 * @param {object} config - Configuration du router
 * @param {string} config.stripeSecretKey - Clé secrète Stripe
 * @param {string} config.stripeWebhookSecret - Secret webhook Stripe
 * @param {string} config.frontendUrl - URL du frontend
 * @param {object} config.database - Instance de la base D1
 * @returns {object} Router configuré
 */
export function createBillingRouter(config) {
  const { stripeSecretKey, stripeWebhookSecret, frontendUrl, database } = config;

  // Créer un objet env pour les contrôleurs
  const env = {
  ...
  // Retourner l'interface publique du router
  return {
    handleRequest
  };
}
