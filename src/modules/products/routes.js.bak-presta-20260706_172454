...
async function updateProduct(request, env, tenantId, userId, productId) {
  try {
    ...
    `).bind(
      ...
      body.assignment_type !== undefined ? body.assignment_type : (existing.assignment_type || 'shared'),
      ...
      body.short_description !== undefined ? body.short_description : existing.short_description,
      body.price !== undefined ? body.price : existing.price,
      body.price_currency !== undefined ? body.price_currency : (existing.price_currency || 'EUR'),
      body.compare_at_price !== undefined ? body.compare_at_price : existing.compare_at_price,
      body.stock_quantity !== undefined ? body.stock_quantity : (existing.stock_quantity || 0),
      body.stock_status !== undefined ? body.stock_status : (existing.stock_status || 'in_stock'),
      body.available !== undefined ? body.available : (existing.available !== undefined ? existing.available : 1),
      body.attributes !== undefined ? JSON.stringify(body.attributes) : (existing.attributes || '{}'),
      body.images !== undefined ? JSON.stringify(body.images) : (existing.images || '[]'),
      body.videos !== undefined ? JSON.stringify(body.videos) : (existing.videos || '[]'),
      body.location !== undefined ? JSON.stringify(body.location) : (existing.location || '{}'),
      body.keywords !== undefined ? body.keywords : existing.keywords,
      body.tags !== undefined ? JSON.stringify(body.tags) : (existing.tags || '[]'),
      body.has_variants !== undefined ? body.has_variants : (existing.has_variants || 0),
      body.variants !== undefined ? JSON.stringify(body.variants) : (existing.variants || '[]'),
      body.status !== undefined ? body.status : (existing.status || 'active'),
