...
// Récupérer les prestations du tenant (source unique : products type='service',
// fallback services pendant la transition — helper listPrestations).
async function handleGetServices(request, env, path) {
  try {
    const tenantId = path.split('/')[4];
    const services = await listPrestations(env, tenantId);
